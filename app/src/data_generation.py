...
def make_test_df(samp):
    cust = pd.read_parquet('../resources/data/customers_tm1_e.parquet')
    trans = pd.read_parquet('../resources/data/transactions_tm1_e.parquet')
...
def initial_processing(cust,trans):
    ...
    if type(cust) == str:
        cust = pd.read_parquet(cust)
    if type(trans) == str:
        trans = pd.read_parquet(trans)
...
def monthly_data(df,econ_data_file_name):
    ...
    macro = pd.read_parquet(econ_data_file_name)
    ...
    df_econ = df_econ.drop(['Unnamed: 0'], axis = 1)
    # df_econ.to_parquet('../data/monthly_churn_with_econV2.parquet')
    return df_econ
...
def final_processing(df):
    ...
    df_ = df.copy()
    del df
    # df_.to_parquet('../../data/Modelling_DATA.parquet', compression = 'gzip')
    return df_

if __name__ == '__main__':

    cust_file_name = '../resources/data/customers_tm1_e.parquet'
    trans_file_name = '../resources/data/transactions_tm1_e.parquet'
    econ_data_file_name = '../resources/data/macroecon_features.parquet'
